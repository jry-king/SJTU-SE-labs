...
private:
  ...
  long heartbeatCounter;							// record how many heartbeats have taken place
  std::set<blockid_t> writtenBlocks;
  std::map<DatanodeIDProto, int> datanodes;			// registered datanodes and its heartbeat number(used to check liveness)
  void heartbeatCount();							// count number of heartbeats
private:
  void GetFileInfo();
