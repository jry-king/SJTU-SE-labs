...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if ((uintptr_t)va >= UTOP || PGOFF(va) || !(perm & PTE_U) || !(perm & PTE_P) || (perm & ~PTE_SYSCALL))
	{
		return -E_INVAL;
	}

	struct PageInfo *pp;
	if (!(pp = page_alloc(ALLOC_ZERO)))			// content of new page is set to 0 in page_alloc
	{
		return -E_NO_MEM;
	}
	if (page_insert(e->env_pgdir, pp, va, perm) < 0)	// mapped page is unmapped in page_insert if there was one
	{
		page_free(pp);
		return -E_NO_MEM;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if ((uintptr_t)srcva >= UTOP || PGOFF(srcva) || (uintptr_t)dstva >= UTOP || PGOFF(dstva) || !(perm & PTE_U) || !(perm & PTE_P) || !(perm & PTE_SYSCALL))
	{
		return -E_INVAL;
	}
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	page_remove(e->env_pgdir, va);		// page_remove will do nothing if no page is mapped
	return 0;
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (srcva < (void *)UTOP)	// also send a page mapping
	{
