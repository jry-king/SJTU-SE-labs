...
trap_init_percpu(void)
{
	...
	int currentcpu = cpunum();

	// Setup a TSS so that we get the right stack
	// when we trap to the kernel.
	thiscpu->cpu_ts.ts_esp0 = KSTACKTOP - currentcpu * (KSTKSIZE + KSTKGAP);
	thiscpu->cpu_ts.ts_ss0 = GD_KD;
	thiscpu->cpu_ts.ts_iomb = sizeof(struct Taskstate);

	// Initialize the TSS slot of the gdt.
	gdt[(GD_TSS0 >> 3) + currentcpu] = SEG16(STS_T32A, (uint32_t) (&(thiscpu->cpu_ts)),
					sizeof(struct Taskstate) - 1, 0);
	gdt[(GD_TSS0 >> 3) + currentcpu].sd_s = 0;

	// Load the TSS selector (like other segment selectors, the
	// bottom three bits are special; we leave them 0)
	ltr(GD_TSS0 + (currentcpu << 3));
...
trap_dispatch(struct Trapframe *tf)
{
	...
	{
		case T_DIVIDE:
			cprintf("In trap_dispatch: trap T_DIVIDE, divide 0 error\n");
			break;
		case T_GPFLT:
			cprintf("In trap_dispatch: trap T_GPFLT, general protection fault\n");
			break;
		case T_PGFLT:
		...
		default:
		{
			...
			{
				cprintf("In trap_dispatch: unhandled trap No.%d\n", tf->tf_trapno);
			}
			break;
		}
	}
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if (curenv->env_pgfault_upcall)
	{
		...
		if (tf->tf_esp < UXSTACKTOP && tf->tf_esp >= (UXSTACKTOP - PGSIZE))		// recursive page fault, use sizeof(void *) to represent a one-word space
		{
		...
		// run page fault handler on exception stack
		curenv->env_tf.tf_eip = (uintptr_t)curenv->env_pgfault_upcall;
		curenv->env_tf.tf_esp = (uintptr_t)utf;
		env_run(curenv);
	}

	// Destroy the environment that caused the fault if no page fault handler.
