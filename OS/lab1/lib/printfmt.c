	 ...
	 unsigned long long num, unsigned base, int width, int padc)
{
	...
	if (padc == '-')
	{
		...
		int temp = num;
		for(; temp >= base; temp /= base)
		{
			width--;
		}
		// print the number
		printnum(putch, putdat, num, base, 0, ' ');
		// print padding, left adjusted number can only be padded by space
		while (--width > 0)
		{
			putch(' ', putdat);
		}
	...
	if (num >= base) {
		...
		// precede right aligned positive number with + if needed
		if (padc == '*')
		{
			padc = ' ';
			while (--width > 0)
...
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap)
{
	...
	reswitch:
		...
		case 'x':
			...
			base = 16;
		number:
			// precede left adjusted positive number with +
			// or right aligned number padded with 0
			// + or - are not counted into width here
			if (signflag && (padc == '-' || padc == '0'))
			{
				putch('+', putdat);
			}
			// add a special padc to deal with + in right aligned number padded with space
			else if (signflag && padc == ' ')
			{
				padc = '*';
			}
		...
		// doesn't provide information to printf
		// instead store this information in the argument
		case 'n':
		{
			...
			// Your code here
			char *charNum = va_arg(ap, char *);
			// NULL pointer, handle error like in %e specifier
			if (!charNum)
			{
